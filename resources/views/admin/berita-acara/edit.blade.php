...
<div class="max-w-4xl space-y-6">
    <div>
        ...
        <h1 class="font-heading font-bold text-2xl text-lsp-text mt-4">Edit Berita: {{ $berita->judul }}</h1>
        <p class="text-xs text-slate-600">Perbarui isi tulisan, kategori, status terbit, atau ganti cover utama.</p>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm">
        <form action="{{ route('admin.berita.update', $berita->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="judul" id="judul" value="{{ old('judul', $berita->judul) }}" required class="w-full bg-white border @error('judul') border-rose-500 @else border-slate-200 @enderror rounded-xl px-4 py-3 text-xs text-lsp-text focus:outline-none focus:border-lsp-accent">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="kategori" id="kategori" value="{{ old('kategori', $berita->kategori) }}" required class="w-full bg-white border @error('kategori') border-rose-500 @else border-slate-200 @enderror rounded-xl px-4 py-3 text-xs text-lsp-text focus:outline-none focus:border-lsp-accent">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="penulis" id="penulis" value="{{ old('penulis', $berita->penulis) }}" required class="w-full bg-white border @error('penulis') border-rose-500 @else border-slate-200 @enderror rounded-xl px-4 py-3 text-xs text-lsp-text focus:outline-none focus:border-lsp-accent">
                ...
                <div class="space-y-2">
                    ...
                    <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $berita->tanggal->format('Y-m-d')) }}" required class="w-full bg-white border @error('tanggal') border-rose-500 @else border-slate-200 @enderror rounded-xl px-4 py-3 text-xs text-lsp-text focus:outline-none focus:border-lsp-accent">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <select name="status" id="status" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-xs text-lsp-text focus:outline-none focus:border-lsp-accent">
                        ...
                        <option value="published" {{ old('status', $berita->status) == 'published' ? 'selected' : '' }}>Published (Terbit Publik)</option>
                    </select>
                </div>

                <div class="space-y-2">
                    <label for="thumbnail" class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Ganti Gambar Cover (Maks 2MB)</label>
            ...
            @if($berita->thumbnail)
                <div class="space-y-2">
                    <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500">Cover Saat Ini</span>
                    <div class="h-32 w-52 rounded-xl overflow-hidden border border-slate-200">
                        <img src="{{ $berita->thumbnail }}" alt="" class="object-cover h-full w-full">
                    </div>
                </div>
            @endif
